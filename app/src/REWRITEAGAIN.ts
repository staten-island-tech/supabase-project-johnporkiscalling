...
class WorldGenerator extends Random {
  ...
  getColumnData(
    ...
    if (continentalness < this.cThresh) {
      ...
      if (effectiveTemperature < 0.15) {
        // Very Cold
        if (landElevationFactor > 0.5 && mountainInfluence > this.mFactor * 0.8)
          biome = BiomeData[BIOME_IDS.SNOWY_PEAKS];
        else biome = BiomeData[BIOME_IDS.TAIGA];
      } else if (effectiveTemperature < 0.4) {
        // cold
        if (landElevationFactor > 0.4 && mountainInfluence > this.mFactor * 0.7)
          biome = BiomeData[BIOME_IDS.MOUNTAINS];
        else biome = BiomeData[BIOME_IDS.TAIGA];
      } else if (effectiveTemperature < 0.7) {
        // average
        if (landElevationFactor > 0.5 && mountainInfluence > this.mFactor)
          biome = BiomeData[BIOME_IDS.MOUNTAINS];
        ...
        //HOT
        if (humidity < 0.15) {
          if (landElevationFactor > 0.3 && mountainInfluence > this.mFactor * 0.5)
            biome = BiomeData[BIOME_IDS.BADLANDS];
      ...
      if (
        distToWater >= 0 &&
        distToWater < 1.0 &&
        biome.id !== BIOME_IDS.OCEAN &&
        biome.id !== BIOME_IDS.MOUNTAINS &&
        biome.id !== BIOME_IDS.SNOWY_PEAKS
      ) {
      ...
      baseHeight =
        this.seaLevel + biome.baseHeightOffset + landElevationFactor * biome.heightVariation * 0.5;
      baseHeight += (detailInfluence * 2 - 1) * biome.heightVariation * 0.5; // detailInfluence from [0,1] to [-1,1]

      if (
        biome.id === BIOME_IDS.MOUNTAINS ||
        biome.id === BIOME_IDS.SNOWY_PEAKS ||
        biome.id === BIOME_IDS.BADLANDS
      ) {
...
class VoxelChunk {
  ...
  getVoxel(x: number, y: number, z: number) {
    //operates on a chunk local approach
...
export class ChunkManager {
  //optimize this things memory usage
  ...
  generateChunkMesh(chunk: VoxelChunk) {
    ...
    for (
      let a = -1;
      a < 2;
      a++ //this loop helps ensure that the neighboring chunks are loaded
    ) {
      for (let b = -1; b < 2; b++) {
  ...
  getVoxel(x: number, y: number, z: number) {
    ...
    if (
      !this.chunks.has(`${chunkCords[0]},${chunkCords[1]},${chunkCords[2]}`) &&
      !this.loadedChunks.has(`${chunkCords[0]},${chunkCords[2]}`)
    ) {
