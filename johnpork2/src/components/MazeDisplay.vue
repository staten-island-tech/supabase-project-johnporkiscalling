<template>
  ...
  <script setup lang="ts">
    ...
    import { initializeContainer } from '../mazealgo';
    import { wallCheck } from '../utils';
    const container = initializeContainer(width,height,length);
    console.log(container[0][0][0])
    const axisReference = [
      {
          maxWall:"right",
          minWall:"left"
  ...
  function manipulateWalls(a:Array<number>,b:Array<number>)
  {
      ...
      const wallReference = axisReference[axis];
    ...
    function test()
    {
      for(let x = 0; x<3; x++)
      {
        for(let y = 0; y<3; y++)
        {
          ...
          {
            if(x==1||z==1){continue};
            let position =  new THREE.Vector3(x+.5,y+.5,z+.5);
